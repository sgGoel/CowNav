...
def get_pointcloud(color, depth, intrinsics, w2c, transform_pts=True, 
                   mask=None, compute_mean_sq_dist=False, mean_sq_dist_method="projective", seg=None):
    ...
    if mask is not None:
        if seg is None:
            ...
            point_label = torch.zeros_like(point_cld)[:, 0]# H*W,
            for cls_i in range(seg.shape[-1]): 
                seg_cls = (seg[..., cls_i] > 0).reshape(-1)
                point_label[seg_cls] = cls_i + 1
            point_cld = point_cld[mask]
            point_label = point_label[mask]
        if compute_mean_sq_dist:
    ...
    if seg is None:
        ...
        if compute_mean_sq_dist:
            return point_cld, mean3_sq_dist, point_label
        else:
            return point_cld, point_label


def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, seg=None):
...
def initialize_optimizer(params, lrs_dict, tracking):
    lrs = lrs_dict
    param_groups = [{'params': [v], 'name': k, 'lr': lrs[k]} for k, v in params.items() if k != 'labels']
    if tracking:
...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, mean_sq_dist_method, densify_dataset=None):
    ...
    color, depth, seg, intrinsics, pose = dataset[0]
    ...
    mask = mask.reshape(-1)
    init_pt_cld, mean3_sq_dist, init_pt_label = get_pointcloud(color, depth, densify_intrinsics, w2c, 
                                                mask=mask, compute_mean_sq_dist=True, 
                                                mean_sq_dist_method=mean_sq_dist_method, seg=seg)

    # Initialize Parameters
    params, variables = initialize_params(init_pt_cld, num_frames, mean3_sq_dist, init_pt_label)
...
def initialize_new_params(new_pt_cld, mean3_sq_dist, seg=None):
...
def add_new_gaussians(params, variables, curr_data, sil_thres, time_idx, mean_sq_dist_method, seg=None):
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        new_pt_cld, mean3_sq_dist, point_label = get_pointcloud(curr_data['im'], curr_data['depth'], curr_data['intrinsics'], 
                                    curr_w2c, mask=non_presence_mask, compute_mean_sq_dist=True,
                                    mean_sq_dist_method=mean_sq_dist_method, seg=seg)
        new_params = initialize_new_params(new_pt_cld, mean3_sq_dist, point_label)
...
def rgbd_slam(config: dict):
    ...
    if config['load_checkpoint']:
        ...
        for time_idx in range(checkpoint_time_idx):
            ...
            color, depth, seg, _, gt_pose = dataset[time_idx]
            # Process poses
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            if config['mapping']['add_new_gaussians'] and time_idx > 0:
                ...
                params, variables = add_new_gaussians(params, variables, densify_curr_data, 
                                                      ...
                                                      config['mean_sq_dist_method'], seg=seg)
...
if __name__ == "__main__":
    ...
    # dataset_dir = '/instance_imagenav/end2end_imagenav/env_collect'
    # scenes_list = os.listdir(dataset_dir)
    # for scene in scenes_list:
    #     if os.path.isdir(os.path.join(dataset_dir, scene)):
    #         experiment.config['data']['basedir'] = dataset_dir
    #         experiment.config['data']['sequence'] = scene
    #         experiment.config["run_name"] = scene
    #         print("***************", scene, "***************")
